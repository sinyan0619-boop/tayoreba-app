...
import { CaseRank, CaseStatus, STATUS_COLORS } from '@/types';
      ...
      <div className="bg-white border-b border-gray-200 px-3 pt-2 pb-2 space-y-1.5 shrink-0">
        <div className="flex gap-1.5 flex-wrap">
          {STATUSES.map((s) => (
            <button
              key={s}
              onClick={() => setSelStatuses((prev) => toggle(prev, s))}
        ...
        <div className="flex items-center gap-1.5">
          {RANKS.map((r) => (
            <button
              key={r}
              onClick={() => setSelRanks((prev) => toggle(prev, r))}
          ...
          <span className="ml-auto text-xs text-gray-500">{filtered.length}件</span>
        </div>
      </div>
